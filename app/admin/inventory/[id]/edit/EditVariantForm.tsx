...
export default function EditVariantForm({ initialVariant }: EditVariantFormProps) {
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [categoryType, setCategoryType] = useState(initialVariant.product.categoryType);
  const [imageFile, setImageFile] = useState<File | null>(null);
  const [imagePreviewUrl, setImagePreviewUrl] = useState(initialVariant.imageUrl || "");
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    formData.set("variantId", initialVariant.id.toString());
    formData.set("productId", initialVariant.productId.toString());
    ...
    <div className="p-6 md:p-10 space-y-8 max-w-5xl mx-auto">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl md:text-3xl font-heading font-bold text-brand-black">Edit Product Variant</h1>
            <p className="text-brand-textMuted text-sm mt-1">Update details and stock for this specific variant.</p>
          </div>
        </div>
      </div>
        ...
        <div className="grid grid-cols-1 xl:grid-cols-3 gap-8">
          ...
          <div className="xl:col-span-2 space-y-8">
            <div className="bg-brand-white border border-brand-border rounded-2xl p-6 md:p-8 shadow-sm">
              <h2 className="text-lg font-bold text-brand-black mb-6 border-b border-brand-border pb-4">1. Product Blueprint Details</h2>
              ...
              <div className="space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="space-y-2">
                    ...
                    <input type="text" name="brandName" defaultValue={initialVariant.product.brandName} required className="w-full bg-gray-50 border border-brand-border rounded-xl px-4 py-3 text-sm text-brand-black focus:outline-none focus:border-brand-gold focus:ring-1 focus:ring-brand-gold transition-all" />
                  </div>
                  
                  <div className="space-y-2">
                    <label className="text-xs font-bold text-brand-textMuted uppercase tracking-wider block ml-1">Model Name</label>
                    <input type="text" name="modelName" defaultValue={initialVariant.product.modelName} required className="w-full bg-gray-50 border border-brand-border rounded-xl px-4 py-3 text-sm text-brand-black focus:outline-none focus:border-brand-gold focus:ring-1 focus:ring-brand-gold transition-all" />
                  </div>
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <textarea name="baseDescription" defaultValue={initialVariant.product.baseDescription} required rows={4} className="w-full bg-gray-50 border border-brand-border rounded-xl px-4 py-3 text-sm text-brand-black focus:outline-none focus:border-brand-gold focus:ring-1 focus:ring-brand-gold transition-all"></textarea>
                </div>
              </div>
            </div>

            <div className="bg-brand-white border border-brand-border rounded-2xl p-6 md:p-8 shadow-sm">
              <h2 className="text-lg font-bold text-brand-black mb-6 border-b border-brand-border pb-4">2. Variant Specifics</h2>
              
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div className="space-y-2">
                  <label className="text-xs font-bold text-brand-textMuted uppercase tracking-wider block ml-1">SKU Code</label>
                  <input type="text" name="skuString" defaultValue={initialVariant.skuString} required className="w-full bg-gray-50 border border-brand-border rounded-xl px-4 py-3 text-sm text-brand-black focus:outline-none focus:border-brand-gold focus:ring-1 focus:ring-brand-gold transition-all" />
                </div>
                
                <div className="space-y-2">
                  <label className="text-xs font-bold text-brand-textMuted uppercase tracking-wider block ml-1">Price (PHP)</label>
                  <input type="number" step="0.01" name="price" defaultValue={(initialVariant.priceCents / 100).toFixed(2)} required className="w-full bg-gray-50 border border-brand-border rounded-xl px-4 py-3 text-sm text-brand-black focus:outline-none focus:border-brand-gold focus:ring-1 focus:ring-brand-gold transition-all" />
                </div>
                ...
                <div className="space-y-3">
                  <label className="text-xs font-bold text-brand-textMuted uppercase tracking-wider block ml-1">Stock Quantity</label>
          ...
          <div className="space-y-8">
            <div className="bg-brand-white border border-brand-border rounded-2xl p-6 md:p-8 shadow-sm sticky top-24">
              ...
              <div className="space-y-6">
                ...
                <div className="space-y-2">
                  <label className="text-xs font-bold text-brand-textMuted uppercase tracking-wider block ml-1">Update Image</label>
